...
async def get_upcoming_birthdays_endpoint(
    days: int = Query(30, ge=1, le=365, description="Number of days ahead to check for birthdays (1-365)."),
    ...
    try:
        ...
        logger.info(f"Successfully retrieved {len(upcoming_birthdays)} upcoming birthdays from service.")
        ...
        logger.exception("An unexpected error occurred while fetching upcoming birthdays.", exc_info=True)
        raise HTTPException(
