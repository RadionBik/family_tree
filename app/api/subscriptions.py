...
from app.schemas.subscription import SubscriptionCreate, SubscriptionResponse, SubscriptionRead # Schemas
from app.services import subscription_service # Service function
from app.services.subscription_service import EmailAlreadyExistsError # Custom exception
...
async def subscribe_email(
    ...
    """
    logger.info(f"Received request for /subscribe endpoint with email: {subscription_in.email}")
    try:
        new_subscription_orm = await subscription_service.add_subscription(db, subscription_in)
        ...
        return SubscriptionResponse(
            message=get_text("subscription_successful"),
            subscription=subscription_read
        )
    except EmailAlreadyExistsError:
        logger.warning(f"Attempt to subscribe already existing email: {subscription_in.email}")
        raise HTTPException(
        ...
        logger.exception(f"An unexpected error occurred during subscription for {subscription_in.email}.", exc_info=True)
        raise HTTPException(
