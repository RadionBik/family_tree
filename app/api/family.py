...
from app.schemas.family import (
    FamilyMemberRead, FamilyMemberCreate, FamilyMemberUpdate, MemberListDelete,
    RelationRead, RelationCreate,
...
from app.services.family_service import MemberNotFoundError, RelationNotFoundError, InvalidRelationError # Import custom exceptions
...
async def list_family_members(
    ...
    search: Optional[str] = Query(None, description="Search term to filter by name"), # Add search query param
    ...
    """
    logger.info(f"Admin '{current_user.username}' listing members: page={page}, size={size}, search='{search}'")
    skip = (page - 1) * size
    limit = size
    try:
        members_list, total_items = await family_service.get_paginated_family_members(
            db=db, skip=skip, limit=limit, search_term=search # Pass search term
        )
        ...
        logger.exception(f"Error listing members for admin '{current_user.username}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def create_family_member(
    ...
    """
    logger.info(f"Admin '{current_user.username}' attempting to create family member: {member_data.first_name} {member_data.last_name}")
    try:
        new_member = await family_service.create_family_member(db=db, member_data=member_data)
        logger.info(f"Successfully created member ID {new_member.id} by admin '{current_user.username}'")
        return new_member
    except Exception as e:
        logger.exception(f"Error creating family member by admin '{current_user.username}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_family_member(
    ...
    """
    logger.info(f"Admin '{current_user.username}' attempting to fetch member ID: {member_id}")
    try:
        member = await family_service.get_member_by_id(db=db, member_id=member_id)
        logger.info(f"Successfully fetched member ID {member_id} for admin '{current_user.username}'")
        return member
    except MemberNotFoundError:
        logger.warning(f"Member ID {member_id} not found when requested by admin '{current_user.username}'.")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=get_text("error_member_not_found") # Add this key to locales
        )
    except Exception as e:
        logger.exception(f"Error fetching member ID {member_id} by admin '{current_user.username}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_family_member(
    ...
    """
    logger.info(f"Admin '{current_user.username}' attempting to update member ID: {member_id}")
    try:
        updated_member = await family_service.update_family_member(db=db, member_id=member_id, member_data=member_data)
        logger.info(f"Successfully updated member ID {member_id} by admin '{current_user.username}'")
        return updated_member
    except MemberNotFoundError:
        logger.warning(f"Attempt to update non-existent member ID {member_id} by admin '{current_user.username}'.")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=get_text("error_member_not_found")
        )
    except Exception as e:
        logger.exception(f"Error updating member ID {member_id} by admin '{current_user.username}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_family_member(
    ...
    """
    logger.info(f"Admin '{current_user.username}' attempting to delete member ID: {member_id}")
    try:
        await family_service.delete_family_member(db=db, member_id=member_id)
        logger.info(f"Successfully deleted member ID {member_id} by admin '{current_user.username}'")
        # Return Response with 204 status explicitly for clarity
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    except MemberNotFoundError:
        logger.warning(f"Attempt to delete non-existent member ID {member_id} by admin '{current_user.username}'.")
        ...
        logger.exception(f"Error deleting member ID {member_id} by admin '{current_user.username}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def batch_delete_family_members(
    ...
    logger.info(f"Admin '{current_user.username}' attempting to batch delete members with IDs: {member_ids}")
    try:
        deleted_count = await family_service.delete_multiple_family_members(db=db, member_ids=member_ids)
        logger.info(f"Batch delete completed by admin '{current_user.username}'. Deleted {deleted_count} members.")
        # Return the count of deleted items
        return {"deleted_count": deleted_count, "message": get_text("success_batch_delete", count=deleted_count)} # Add this key
    except Exception as e:
        logger.exception(f"Error during batch delete by admin '{current_user.username}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def create_relationship(
    ...
    """
    logger.info(f"Admin '{current_user.username}' attempting to create relationship: {relation_data.from_member_id} -> {relation_data.to_member_id} ({relation_data.relation_type})")
    try:
        new_relation = await family_service.create_relationship(db=db, relation_data=relation_data)
        logger.info(f"Successfully created relationship ID {new_relation.id} by admin '{current_user.username}'")
        return new_relation
    except (MemberNotFoundError, InvalidRelationError) as e:
        logger.warning(f"Failed to create relationship by admin '{current_user.username}': {e}")
        ...
        logger.exception(f"Error creating relationship by admin '{current_user.username}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_relationship(
    ...
    """
    logger.info(f"Admin '{current_user.username}' attempting to delete relationship ID: {relation_id}")
    try:
        await family_service.delete_relationship(db=db, relation_id=relation_id)
        logger.info(f"Successfully deleted relationship ID {relation_id} by admin '{current_user.username}'")
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    except RelationNotFoundError as e:
        logger.warning(f"Attempt to delete non-existent relationship ID {relation_id} by admin '{current_user.username}'.")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=str(e) # Use the exception's localized message
        )
    except Exception as e:
        logger.exception(f"Error deleting relationship ID {relation_id} by admin '{current_user.username}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
