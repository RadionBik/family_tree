...
const FamilyTree = ({ selectedMemberId, onMemberSelect }) => {
  ...
  const renderMemberDetails = () => {
    ...
    if (!memberData) {
      console.warn(`Member data not found in elements for ID: ${selectedMemberId}`);
      return null;
    }
    ...
    return (
      <Paper elevation={1} sx={{ marginTop: 3, padding: 2 }}>
        ...
        <Typography variant="body1" gutterBottom>
          <Typography component="span" fontWeight="bold">{t('name')}:</Typography> {memberData.label}
        </Typography>
        {memberData.birth_date && (
          <Typography variant="body1" gutterBottom>
            <Typography component="span" fontWeight="bold">{t('birthDate')}:</Typography> {memberData.birth_date}
          </Typography>
        )}
        {memberData.death_date && (
          <Typography variant="body1" gutterBottom>
            <Typography component="span" fontWeight="bold">{t('deathDate')}:</Typography> {memberData.death_date}
          </Typography>
        )}
        {memberData.gender && (
          <Typography variant="body1" gutterBottom>
            <Typography component="span" fontWeight="bold">{t('genderLabel')}:</Typography> {t(`gender.${memberData.gender}`, memberData.gender)}
          </Typography>
        )}
        {memberData.notes && (
          <Typography variant="body1" gutterBottom>
            <Typography component="span" fontWeight="bold">{t('notes')}:</Typography> {memberData.notes}
          </Typography>
        )}
        <Button variant="outlined" size="small" onClick={() => onMemberSelect(null)} sx={{ marginTop: 1 }}>
          {t('close')}
        </Button>
      </Paper>
    );
    ...
    <Paper elevation={2} sx={{ p: 2, mb: 2 }}>
      ...
      {!loading && !error && elements.length > 0 && (
        <>
          ...
          <Box sx={{ pl: 1 }}> {/* Optional: Slight padding-left for legend */}
            <GraphLegend />
          </Box>
        </>
      )}
