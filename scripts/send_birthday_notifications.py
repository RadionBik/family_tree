...
from app.utils.database import AsyncSessionFactory, async_engine # Import session factory and engine
...
async def run_notifications():
        ...
        async with AsyncSessionFactory() as session:
            ...
            try:
                ...
                if not birthday_infos:
                    logger.info("No birthdays today or no subscribers. Nothing to send.")
                    return
                ...
                for info in birthday_infos:
                    if not info.subscriber_emails:
                        logger.warning(f"No subscribers found for {info.name}'s birthday notification. Skipping.")
                        continue

                    logger.info(f"Processing birthday for {info.name} (Age: {info.age}). Subscribers: {len(info.subscriber_emails)}")
                    total_recipients += len(info.subscriber_emails)

                    subject, body = format_birthday_email(info.name, info.age)

                    success = send_email(subject, body, info.subscriber_emails, app_config)

                    if success:
                        logger.info(f"Successfully sent birthday notification for {info.name} to {len(info.subscriber_emails)} recipients.")
                        emails_sent_successfully += 1
                    else:
                        logger.error(f"Failed to send birthday notification for {info.name}.")

            except Exception as e:
                logger.exception("Error during database operation or email processing.", exc_info=True)
            finally:
    ...
    logger.info(f"Summary: Found={birthdays_found}, Notifications Attempted={emails_sent_successfully}, Total Recipients Targeted={total_recipients}")
...
if __name__ == "__main__":
    ...
    if missing_vars:
        logger.error(f"Missing required environment variables for email: {', '.join(missing_vars)}")
        logger.error("Please set them in your environment or .env file.")
        sys.exit(1)
    # Optional check for credentials if not using an open relay
    if not os.getenv('MAIL_SERVER') == 'localhost': # Example check if not using local relay
         if not os.getenv('MAIL_USERNAME') or not os.getenv('MAIL_PASSWORD'):
              logger.warning("MAIL_USERNAME or MAIL_PASSWORD not set. Email sending might fail if authentication is required.")
