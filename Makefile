...
.PHONY: help setup-local run-local-backend run-local-frontend run-local docker-run lint clean-pycache clean-node clean
...
help:
	...
	@echo "  docker-run           - Build and run the application using Docker Compose"
...
docker-run: .env docker-compose.yml Dockerfile frontend/Dockerfile
